...
- (id) init {
	if (self = [super init]) {
        connectionRequestMap = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, 
...
- (void)beginDownload:(NSURL *)url delegate:(id)delegateObj {
    ...
    if ([delegateObj respondsToSelector:@selector(download:failedWithError:)]) {
        ...
        NSLog(@"WARNING: %@ does not respond to download:failedWithError:",[delegateObj class]);
    }
}
...
-(void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
	...
	if (dl.receivedData != nil) {
        ...
        NSLog(@"WARNING: DownloadManager got response from unknown connection: %@",dl.url);
    }
}
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if ([dl.delegate respondsToSelector:@selector(download:failedWithError:)]) {
        ...
        NSLog(@"WARNING: %@ does not respond to download:failedWithError:",[dl.delegate class]);
    }
    ...
    @synchronized(self) {
        ...
        [UIApplication sharedApplication].networkActivityIndicatorVisible = CFDictionaryGetCount(connectionRequestMap) > 0;
    }
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if ([dl.delegate respondsToSelector:@selector(download:completedWithData:)]) {
        ...
        NSLog(@"WARNING: %@ does not respond to download:completedWithData:",[dl.delegate class]);
    }
